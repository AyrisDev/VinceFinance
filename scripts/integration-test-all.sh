...
## default port prefixes for vinced
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    ...
    "$("$PWD"/build/vinced keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000avince,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/vinced gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/vinced start --pruning=nothing --rpc.unsafe \
    ...
    VINCE_PID=$!
    echo "started vince node, pid=$VINCE_PID"
    # add PID to array
    arr+=("$VINCE_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test vince node $HOST_RPC ..."
...
stop_func() {
    VINCE_PID=$i
    echo "shutting down node, pid=$VINCE_PID ..."
