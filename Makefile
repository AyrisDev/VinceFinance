...
ifeq (, $(shell which gencodec))
	...
	@go get github.com/fjl/gencodec@latest
else
...
ifeq (, $(shell which protoc-gen-go))
	...
	@go get github.com/fjl/gencodec github.com/golang/protobuf/protoc-gen-go@latest
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(VINCE_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(VINCE_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(VINCE_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(VINCE_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(VINCE_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\vince\Z vinced/node "./vinced testnet --v 4 -o /vince --keyring-backend=test --ip-addresses vincednode0,vincednode1,vincednode2,vincednode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(VINCE_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/vince:Z vinced/node "./vinced testnet --v 4 -o /vince --keyring-backend=test --ip-addresses vincednode0,vincednode1,vincednode2,vincednode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\vinced:vince\Z vinced/node "./vinced unsafe-reset-all --home=/vince"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\vinced:vince\Z vinced/node "./vinced unsafe-reset-all --home=/vince"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\vinced:vince\Z vinced/node "./vinced unsafe-reset-all --home=/vince"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\vinced:vince\Z vinced/node "./vinced unsafe-reset-all --home=/vince"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/vinced:/vince:Z vinced/node "./vinced unsafe-reset-all --home=/vince"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/vinced:/vince:Z vinced/node "./vinced unsafe-reset-all --home=/vince"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/vinced:/vince:Z vinced/node "./vinced unsafe-reset-all --home=/vince"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/vinced:/vince:Z vinced/node "./vinced unsafe-reset-all --home=/vince"
endif
...
.PHONY: build-docker-local-vince localnet-start localnet-stop
